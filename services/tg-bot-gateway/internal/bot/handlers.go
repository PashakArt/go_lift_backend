...
func (s *HTTPServer) HandleGetReport(w http.ResponseWriter, r *http.Request) {
	query := r.URL.Query()

	req := types.ReportQueryParams{
	...
	res, err := s.workoutClient.ExportWorkoutsReport(ctx, userId, fromDate, toDate)
	if err != nil {
		log.Printf("[ERROR] ExportWorkoutsReport gRPC call failed: %v", err)
		RespondWithError(w, http.StatusInternalServerError, "Internal server error")
		return
	}
	...
	w.Header().Set("Content-Length", strconv.Itoa(len(res.GetContent())))
	w.WriteHeader(http.StatusOK)

	if _, err := w.Write(res.GetContent()); err != nil {
		log.Printf("[ERROR] Failed to write excel file stream: %v", err)
	}
}
